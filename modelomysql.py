...
# Datos de conexión a la base de datos. Este es una BD que tenemos en un contenedor de Docker llamado pythonsql.
BD = {
    "database":"Biblioteca",
    "host":"localhost",
    "user":"Biblioteca",
...
def obtiene_registros(tabla):
    ...
    if conn:
        ...
        # lista de resultados.
        registros =[[r[0].capitalize() for r in cur.description]] #[['Id', 'Titulo', 'Editorial', 'Numpag', 'Autores']]
        # Se obtiene la lista de resultados de la consulta SQL
        registros += cur.fetchall()
        # Se cierra la BD
        conn.close()

        return registros #[['Id', 'Titulo', 'Editorial', 'Numpag', 'Autores'], (1, 'Yo, Robot', 'Gnome Press', 374, 1)]....
    else:
...
def actualiza_registro(tabla, id, valores):
    ...
    if conn:
        ...
        registro = [t[0] if t[1] == None else t[1]
            for t in zip(registro, valores)] #Para cada campo, toma el nuevo valor (si no es None), o toma el valor que ya tenía el registro
        # Se obtiene un cursor o indice a la base de datos
        cur = conn.cursor()
        # Se arma una tupla con los valores de los campos
        # Se crea una cadena con tantos símbolos "%s" como valores
